...
BEGIN
  ...
  category_text := CASE
    ...
    ELSE 'Repair Final'
  END;
  transaction_invoice_number_text := CONCAT(
    COALESCE(NULLIF(invoice_number_text, ''), 'repair'),
    '-',
    UPPER(CASE WHEN normalized_stage = 'advance' THEN 'advance' ELSE 'final' END),
    CASE
      WHEN repair_id_text <> '' THEN '-' || repair_id_text
      ELSE ''
    END
  );
  audit_note := FORMAT(
