...
BEGIN
  ...
  transaction_invoice_number_text := CASE
    WHEN NULLIF(invoice_number_text, '') IS NULL THEN ''
    WHEN normalized_stage = 'advance' THEN invoice_number_text || '-A'
    ELSE invoice_number_text || '-F'
  END;
  audit_note := FORMAT(
...
BEGIN
  ...
  WITH repair_targets AS (
    SELECT
      t.id,
      CASE
        WHEN COALESCE(t.notes, '') ~* 'repair_payment_stage=advance' THEN 'A'
        WHEN COALESCE(t.notes, '') ~* 'repair_payment_stage=final' THEN 'F'
        WHEN COALESCE(t.invoice_number, '') ~* '^\d{6}-advance(-|$)' THEN 'A'
        WHEN COALESCE(t.invoice_number, '') ~* '^\d{6}-final(-|$)' THEN 'F'
        ELSE NULL
      END AS stage_code,
      COALESCE(
        substring(COALESCE(t.notes, '') from 'invoice_number=(\d{6})'),
        substring(COALESCE(t.invoice_number, '') from '^(\d{6})')
      ) AS base_invoice
    FROM public.transactions t
    WHERE LOWER(COALESCE(t.source, '')) = 'repair'
  )
  UPDATE public.transactions t
  SET invoice_number = repair_targets.base_invoice || '-' || repair_targets.stage_code
  FROM repair_targets
  WHERE t.id = repair_targets.id
    AND repair_targets.base_invoice IS NOT NULL
    AND repair_targets.stage_code IS NOT NULL
    AND COALESCE(t.invoice_number, '') <> repair_targets.base_invoice || '-' || repair_targets.stage_code
    AND NOT EXISTS (
      SELECT 1
      FROM public.transactions conflict
      WHERE conflict.id <> t.id
        AND conflict.invoice_number = repair_targets.base_invoice || '-' || repair_targets.stage_code
    );

  WITH online_targets AS (
    SELECT
      t.id,
      substring(COALESCE(t.notes, '') from 'Abholschein:(\d{6})') AS base_invoice,
      CASE
        WHEN COALESCE(t.notes, '') ~* 'stage:[[:space:]]*advance' THEN 'A'
        WHEN COALESCE(t.notes, '') ~* 'stage:[[:space:]]*final' THEN 'F'
        ELSE NULL
      END AS stage_code
    FROM public.transactions t
    WHERE LOWER(COALESCE(t.source, '')) = 'online-order'
  )
  UPDATE public.transactions t
  SET invoice_number = online_targets.base_invoice || '-' || online_targets.stage_code
  FROM online_targets
  WHERE t.id = online_targets.id
    AND online_targets.base_invoice IS NOT NULL
    AND online_targets.stage_code IS NOT NULL
    AND COALESCE(t.invoice_number, '') <> online_targets.base_invoice || '-' || online_targets.stage_code
    AND NOT EXISTS (
      SELECT 1
      FROM public.transactions conflict
      WHERE conflict.id <> t.id
        AND conflict.invoice_number = online_targets.base_invoice || '-' || online_targets.stage_code
    );
END
$$;
