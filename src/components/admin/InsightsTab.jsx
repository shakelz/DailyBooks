...
import {
    TrendingUp, DollarSign, Activity, AlertCircle, Calendar, Filter, Zap, Package,
    RefreshCw, BarChart3, Scale, Users, Wrench, ChevronDown, ChevronUp, ChevronRight,
    Wallet, ArrowUpRight, ArrowDownRight, CircleDollarSign, Receipt, Sparkles, X, Clock, CreditCard, Tag
} from 'lucide-react';
...
export default function InsightsTab() {
    ...
    const analytics = useMemo(() => {
        ...
        return {
            ...
            unifiedTotals: unified.totals,
            revenueBreakdown: unified.revenueBreakdown || [],
    ...
    const formattedPeriodLabel = useMemo(() => {
        ...
        const isToday = isSingleDay && rangeStart.toDateString() === new Date().toDateString();
        const options = { day: '2-digit', month: '2-digit', year: 'numeric' };
        if (isToday) {
            return `Heute (${rangeStart.toLocaleDateString('de-DE', options)})`;
        }
        if (isSingleDay) {
            return rangeStart.toLocaleDateString('de-DE', { weekday: 'short', day: '2-digit', month: '2-digit', year: 'numeric' });
        }
    ...
    const dailyFinancialLedger = useMemo(() => {
        ...
        transactions.forEach((txn) => {
            ...
            const isIncomeTxn = String(txn?.type || txn?.tx_type || '').toLowerCase() === 'income' ||
                                String(txn?.source || '').toLowerCase() === 'sale' ||
    ...
    return (
        <div className="space-y-4 animate-in fade-in duration-500 pb-10 max-w-[1500px] mx-auto">
            ...
            <TransactionDetailModal
                isOpen={Boolean(detailModalTxn)}
                onClose={() => setDetailModalTxn(null)}
                txn={detailModalTxn}
            />
        </div>
    );
